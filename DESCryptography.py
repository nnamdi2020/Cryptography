...
# Generate 56-Bit Permutation using the PC-1 Table 
PC1_PERMUTE= [57, 49, 41, 33, 25, 17, 9,
        1, 58, 50, 42, 34, 26, 18,
        10, 2, 59, 51, 43, 35, 27,
        19, 11, 3, 60, 52, 44, 36,
...
PC2_PERMUTE = [14, 17, 11, 24, 1, 5, 3, 28,
...
PI_16RDS = [40, 8, 48, 16, 56, 24, 64, 32,
...
#Converts a hexadecimal to a binary and returns the binary
def convertHex2Binary (hexkey, num_of_bits):
    scale = 16 ## equals to hexadecimal
    binary = bin(int(hexkey, scale))[2:].zfill(num_of_bits)
    return binary
...
# Generate 56-Bit Permutation using the PC-1 Table 
def permutatePC1(key_string):
    """
    Permutes the key string in order to obtain the 56-bit encryption key
    :param key_string: The eight character key string provided by the user
    :return: The permuted 56-bit encryption key based on PC1 Permutation 0table
    """
    
    key = BitVector(bitstring=key_string)
    key = key.permute(pc1_permutation)
...
class desCryptography():
    ...
    def run(self, key, text, action=ENCRYPT, padding=False):
        ...
        for block in text_blocks:  # Loop over all the blocks of data
            ...
            for i in range(16):  # Do the 16 rounds
                d_e = self.expand(d, EXPAND_MATRIX)  # Expand d to match Ki size (48bits)
                if action == ENCRYPT:
            ...
            result += self.permute(d+g, PI_16RDS)
    ...
    def generatekeys(self):  # Algorithm that generates all the keys
        ...
        key = self.permute(key, PC1_PERMUTE)  # Apply the initial permute on the key
        ...
        for i in range(16):  # Apply the 16 rounds
            ...
            self.keys.append(self.permute(tmp, PC2_PERMUTE))
...
#Get Encryption key from  from user input
def getKey(): 
    key = input ('Please enter a key (ex: secret_k): (only 8 characters at this time) ')
    print('\n')
    return key

#Get Encryption plain text from  from user input
def getText(): 
    text = input('Please enter your plain text (only 8 characters at this time)  (example: iLLmatic): ')
    print('\n')
    return text

if __name__ == '__main__':

    print('This program encrypts your plain text using Data Encryption Standard (DES)')
    print('\n')
    key = getKey()
    text = getText()
  
    des = desCryptography()
    plaintext = des.encrypt(key, text)
    ciphertext = des.decrypt(key, plaintext)
    print('plain text: ', text)
    print("Ciphered: %r" % plaintext)
